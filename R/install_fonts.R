...
import_turo_fonts <- function() {
    require("sysfonts")
    font_names = c("basis_grotesque_bold.ttf", "basis_grotesque_medium.ttf", "FreightDispPro-Black.ttf", 
    ...
    for (i in 1:length(font_names)) {
        ...
        if (!file.exists(test_file)) {
            origin = paste0("https://github.com/guleatoma/ggtech/tree/master/Fonts/", 
                font_names[i])
